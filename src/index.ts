  ...
  for (const filePath of chunkedFilePaths) {
    ...
    if (fileExists(outputResponseFilePath)) {
      if (!PROCESS_ALREADY_PROCESSED_FILES) {
        logger.info("[{FileIndex} / {FileLength}] :: OCR output file '{OutputResponseFilePath}' already exists. Skipping OCR processing for this file.", fileIndex, chunkedFilePaths.length, outputResponseFilePath);
        fileIndex++;
        continue;
      }

      logger.info("[{FileIndex} / {FileLength}] :: OCR output file '{OutputResponseFilePath}' already exists. File will be processed again", fileIndex, chunkedFilePaths.length, outputResponseFilePath);
    }
